...
function sayGoodbye(name) {
  return 'Goodbye, ' + name + '. Have a gret day';
}
...
function temperatureInF( tempF , tempC ) {
  ...
  let newTempF = Math.round( (tempC * 9 )/ 5 + 32);

  console.log(newTempF + 'F');
}
temperatureInF(88, 24);
...
function getLastCarInfo(data) {
  const lastCarInfo = data[data.length-1];
...
function getModelYears(data) {
  ...
  for (i = 0 ; i < data.length ; i++) {
    ...
    modelYear.push(data[i].car_year);
    }
  return modelYear;
}
